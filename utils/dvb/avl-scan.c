...
void set_blindscan_mode() {
  ...
  fscanf(bs_mode_handle,"0x%x",cur_bs_mode);
  cur_bs_mode |= (1<<args.frontend);
  fprintf(bs_mode_handle,"0x%.4x",cur_bs_mode);
         ...
         struct dvb_file *chans_file)
{
  ...
  do
  {
    ...
    else
    {
      ...
      //convert frequency to khz and add IF
      p[1].u.data /= 1000;
      p[1].u.data += if_khz;
...
int main(int argc, char *argv[])
{
  ...
  if (args.lnb_name)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < n_freq_bands; i++) {
        printf(_("\t%d: %d MHz to %d MHz, LO %d MHz\n"),
             i,
             lnb_p->freqrange[i].low,
             lnb_p->freqrange[i].high,
             lnb_p->freqrange[i].int_freq);
  ...
  if(!strcmp(info.name, "Availink avl62x1")) {
    ...
    printf("Frontend '%s' not supported.  Only 'Availink avl62x1' and 'Availink avl68x2' supported.");
    exit(1);
  }
  ...
  for (int i = first_band; i <= last_band; i++) {
    ...
    start_freq_hz = (lnb_p->freqrange[i].low -
                    lnb_p->freqrange[i].int_freq) *
                    MILLION;
    start_freq_hz = MAX(info.frequency_min * THOUSAND, start_freq_hz);
    stop_freq_hz = (lnb_p->freqrange[i].high -
                    lnb_p->freqrange[i].int_freq) *
                  MILLION;
    stop_freq_hz = MIN(info.frequency_max * THOUSAND, stop_freq_hz);

    printf(_("\n\nScanning frequency band %d (%d MHz to %d MHz, LO %d MHz)\n"),
             i,
             lnb_p->freqrange[i].low,
             lnb_p->freqrange[i].high,
             lnb_p->freqrange[i].int_freq);
    printf(_("IF from %d MHz to %d MHz\n"),
